...
def reorganize(line, idx_at_the_end): 
    # print(line)
    new_line = []
    idx_at_the_end = 1
    for i in range(len(line)): 
        ...
        if line[i][0] == '.':
            ...
            if len(line[i]) >= len(line[-idx_at_the_end]):
                new_line.append(line[-idx_at_the_end])
                # print(line[-idx_at_the_end])
                new_line.append(['.']*(len(line[i])-len(line[-idx_at_the_end])))
                # print(new_line)
                # exit()
                i = 0
            idx_at_the_end += 1
            return reorganize(new_line, idx_at_the_end)
        else:
